...
class StackAugmentedRNN(nn.Module):
    def __init__(self, input_size, hidden_size, layer_type='GRU',
                 n_layers=1, batch_size=1, is_bidirectional=False, has_stack=False,
        ...
        if self.has_stack:
            rnn_input_size = self.input_size + stack_width
        else:
    ...
    def forward(self, inp, hidden, stack):
        ...
        if self.has_stack:
            ...
            if self.is_bidirectional:
                ...
                hidden_2_stack = hidden_
            if self.n_layers > 1:
                hidden_2_stack = hidden_2_stack[-1]
            ...
            stack_top = stack[:, 0, :].unsqueeze(1).repeat(1,inp.shape[1],1)
